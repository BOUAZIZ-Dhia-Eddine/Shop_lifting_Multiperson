...
class Train_stgcn:
    def __init__(self,csv,features_t, labels_t, class_to_idx_t,features_v,labels_v,
                 class_to_idx_v,batch_size=42,num_class=2,max_persons=3,learning_rate=7e-4 ,epochs=1):
        self.csv_val=csv
    ...
    def initialize_model(self,train_loader):
        ...
        #label_smoothing=0.1,
        weights = torch.tensor([0.73, 1], dtype=torch.float32).to(self.config['device'])
