...
class AttentionPool(nn.Module):
    ...
    def forward(self, x):
        ...
        query = self.query_conv(x).view(N_M, -1, T * V)         # (N_M, C_q, T*V)
        key   = self.key_conv(x).view(N_M, -1, T * V)           # (N_M, C_k, T*V)
        value = self.value_conv(x).view(N_M, -1, T * V)         # (N_M, C,   T*V)

        # Attention map
        attention = torch.bmm(query.permute(0, 2, 1), key)      # (N_M, T*V, T*V)
        attention = F.softmax(attention, dim=-1)                # Normalisation

        # Appliquer l'attention
        out = torch.bmm(value, attention)                       # (N_M, C, T*V)
        out = out.view(N_M, C, T, V)                            # Retour à la forme originale
...
class Model(nn.Module):
    def __init__(self, in_channels, num_class, graph_args, edge_importance_weighting, max_persons=1, **kwargs):
        ...
        self.attn_pool = AttentionPool(in_channels=256)
