...
path_data = "/home/birali/Desktop/Data_check/data/filtred_data_monoprix.csv"
data = pd.read_csv(path_data)


data["class"] = data["class"].replace({4: 0, 7: 0, 10: 0})
data["class"] = data["class"].replace({2:1})
video_clips = data["video_clip"].unique()

np.random.seed(42) 
np.random.shuffle(video_clips)
video_clips=np.random.permutation(video_clips)
np.random.shuffle(video_clips)
video_clips=np.random.permutation(video_clips)
unique_classes = data.groupby(['video_clip', 'person_id'])['class'].first()
total_class_counts  = Counter(unique_classes.values)
...
for vid in video_clips:
    ...
    un_class=video_data.groupby(['video_clip', 'person_id'])['class'].first()
    class_video_counts = Counter(un_class.values)
    ...
    if contains_filled_class:
        ...
        if not val_part.empty:
            ...
            val_un_class=val_part.groupby(['video_clip', 'person_id'])['class'].first()
            val_class_counts += Counter(val_un_class.values)
        if not train_part.empty:
            train_videos.append(vid)
            train_un_class=train_part.groupby(['video_clip', 'person_id'])['class'].first()
            train_class_counts += Counter(train_un_class.values)
        continue
...
train_csv_path = os.path.join(output_dir, "train_splitmonoprix.csv")
val_csv_path = os.path.join(output_dir, "val_splitmonoprix.csv")

train_df.to_csv(train_csv_path, index=False)
val_df.to_csv(val_csv_path, index=False)

path_data = "/home/birali/Desktop/Data_check/data/train_splitmonoprix.csv"
data = pd.read_csv(path_data)
un_class=data.groupby(['video_clip', 'person_id'])['class'].first()

class_video_counts = Counter(un_class.values)
print("*/*/",class_video_counts)
